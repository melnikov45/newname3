...
$lessonTypes = getOptions($dbconn, "SELECT typename FROM eventtools"); // Замените на вашу таблицу
...
$lesson_type = isset($_POST['lesson_type']) ? $_POST['lesson_type'] : '';
$group = isset($_POST['group']) ? $_POST['group'] : '';

// Подготовка и выполнение SQL-запроса
$stmt = pg_prepare($dbconn, "insert_schedule", "INSERT INTO schedule (discipline, classroom, lesson_type, group_name, created_at) VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP)");
$result = pg_execute($dbconn, "insert_schedule", array($discipline, $classroom, $lesson_type, $group));

if (!$result) {
    echo "Ошибка при добавлении записи: " . pg_last_error($dbconn);
}
            ...
            <form>
                ...
                <select id="city-select" name="lesson_type">
                    <?php foreach ($lessonTypes as $lessonType): ?>
                        <option value="<?php echo htmlspecialchars($lessonType['typename']); ?>">
                            <?php echo htmlspecialchars($lessonType['typename']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </form>
...
document.getElementById('submit-button').addEventListener('click', function() {
    ...
    if (selectedCell) {
        ...
        xhr.send(`discipline=${encodeURIComponent(selectedName)}&classroom=${encodeURIComponent(selectedRoom)}&lesson_type=${encodeURIComponent(selectedType)}&group=${encodeURIComponent(selectedGroup)}`);
